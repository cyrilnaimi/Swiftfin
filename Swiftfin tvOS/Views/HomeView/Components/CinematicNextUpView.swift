...
extension HomeView {
    ...
    /// Cinematic hero backed by Next Up — used when nothing is mid-play, so
    /// Home always shows a hero (the closest thing to "continue watching").
    /// Sibling of `CinematicRecentlyAddedView`; labels show the series title +
    /// SxEx only (never the episode name/still), matching the Next Up row.

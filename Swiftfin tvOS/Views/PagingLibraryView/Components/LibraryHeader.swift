...
struct LibraryHeader<ViewModel: ObservableObject & AnyObject>: View {
    ...
    /// Layout mirrors the iOS pattern (`NavigationBarFilterDrawer`): a title
    /// row stacked above a horizontal scroll of capsule-shaped filter pills.
    /// Each pill shows its filter type or current selection summary and opens
    /// its own selector sheet. Active pills tint to the accent color, matching
    /// the tinted-row pattern used in `SettingsView` lists.
    var body: some View {
    ...
    /// pattern in `NavigationBarFilterDrawer`.
...
/// thin material background, stroke outline, accent tint when active. The
/// outer `Button` uses tvOS's default focus appearance (subtle scale) without
/// `.card` so the pill keeps its custom shape rather than getting wrapped in a
/// card chrome.
...
/// Subtle tvOS focus style — scale up on focus (system default ~1.05) without
/// the heavy white card chrome of `.buttonStyle(.card)`. Keeps the capsule
/// shape intact while remaining readable that the pill is interactive.
